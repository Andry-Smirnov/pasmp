...
procedure UInt64ToUInt128(var Dest: TUInt128; const x: qword);
...
procedure UInt128Add(var Dest: TUInt128; const x, y: TUInt128);
{$ifdef caninline}inline;{$endif}
var
  a, b, c, d: qword;
begin
...
procedure UInt128Mul(var Dest: TUInt128; const x, y: TUInt128);
{$ifdef caninline}inline;{$endif}
var
  c, xw, yw, dw: array[0..15] of longword;
  i, j, k: longint;
  v: longword;
begin
  for i := 0 to 15 do
  ...
  Dest.Hi := (qword(dw[0] and $ffff) shl 48) or (qword(dw[1] and $ffff) shl 32) or
    (qword(dw[2] and $ffff) shl 16) or (qword(dw[3] and $ffff) shl 0);
  Dest.Lo := (qword(dw[4] and $ffff) shl 48) or (qword(dw[5] and $ffff) shl 32) or
    (qword(dw[6] and $ffff) shl 16) or (qword(dw[7] and $ffff) shl 0);
end;

procedure UInt128Div64(var Dest: TUInt128; const Dividend: TUInt128; Divisor: qword);
{$ifdef caninline}inline;{$endif}
var
  Quotient: TUInt128;
  Remainder: qword;
...
begin
  ...
  begin
    ...
    if (longword(Remainder shr 32) > longword(Divisor shr 32)) or
      ((longword(Remainder shr 32) = longword(Divisor shr 32)) and
      (longword(Remainder and $ffffffff) >= longword(Divisor and $ffffffff))) then
    begin
...
procedure UInt128Mul64(var Dest: TUInt128; u, v: qword);
{$ifdef caninline}inline;{$endif}
var
  u0, u1, v0, v1, k, t, w0, w1, w2: qword;
begin
  u1 := u shr 32;
...
begin
  ...
  begin
    while (Frequency and $ffffffffe0000000)<>0 do
    begin
...
begin
  ...
  begin
    ...
    while (NowTime+TwoMillisecondsInterval) < EndTime do
    begin
      Sleep(1);
      NowTime := GetTime;
    end;
    while (NowTime+MillisecondInterval) < EndTime do
    begin
      Sleep(0);
      NowTime := GetTime;
    end;
    ...
    NowTime := GetTime;
    EndTime := NowTime + Delay;
    while (NowTime + 4) < EndTime do
    begin
      SDL_Delay(1);
      NowTime := GetTime;
    end;
    while (NowTime + 2) < EndTime do
    begin
      SDL_Delay(0);
      NowTime := GetTime;
    end;
...
begin
  if Frequency <> 0 then
  begin
    if ((Frequency or Time) and int64($ffffffff00000000)) = 0 then
    begin
...
begin
  ...
  begin
    if ((FrameInterval or Time) and int64($ffffffff00000000)) = 0 then
    begin
