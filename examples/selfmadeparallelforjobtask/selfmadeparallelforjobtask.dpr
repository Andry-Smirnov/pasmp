...
type
  TParallelForJobTask = class(TPasMPJobTask)
  private
    fFromIndex:    Longint;
    fToIndex:      Longint;
  ...
  public
    constructor Create(const FromIndex, ToIndex, Granularity, RemainDepth: Longint;
      const Spreaded: Boolean = False);
    destructor Destroy; override;
    procedure Run; override; // run our task
    function Split: TPasMPJobTask; override;
    ...
    function Spread: Boolean; override;
    // (pre-)spread work uniformly into multiple job tasks, the destructor must wait and release the spreaded jobs then.
  end;

  constructor TParallelForJobTask.Create(
  const FromIndex, ToIndex, Granularity, RemainDepth: Longint;
  const Spreaded: Boolean = False);
  begin
    inherited Create;
    FreeOnRelease := True;
  ...
  begin
    if length(fSpreadedJobs) > 0 then
    begin
      GlobalPasMP.WaitRelease(fSpreadedJobs);
      // WaitRelease combines Wait and Release into a single call
  ...
  begin
    ...
    for Index := fFromIndex to fToIndex do
    begin
      Cells[Index] := True;
    end;
  ...
  var
    SplitIndex: Longint;
  begin
    if (fRemainDepth > 0) and (fFromIndex <= fToIndex) and
      (((fToIndex - fFromIndex) + 1) > fGranularity) then
    begin
      SplitIndex := fFromIndex + (((fToIndex - fFromIndex) + 1) div 2);
      Result := TParallelForJobTask.Create(SplitIndex, fToIndex,
        fGranularity, fRemainDepth - 1, True);
  ...
  begin
    if (fRemainDepth > 0) and (fFromIndex <= fToIndex) and
      ((fFromIndex + fGranularity) < fToIndex) then
    begin
  ...
  var
    Count, CountJobs, PartSize, Rest, JobIndex, Size: Longint;
  begin
    Count := ((fToIndex - fFromIndex) + 1);
    if (fRemainDepth > 0) and (fFromIndex <= fToIndex) and
      (Count > fGranularity) and not fSpreaded then
    begin
      ...
      begin
        ...
        if JobIndex < (CountJobs - 1) then
        begin
          fSpreadedJobs[JobIndex] :=
            GlobalPasMP.Acquire(TParallelForJobTask.Create(Index,
            (Index + Size) - 1, fGranularity, fRemainDepth - 1, True));
...
begin
  ...
  try
    GlobalPasMP.Invoke(TParallelForJobTask.Create(1, N, 16, 8));
    // <= Invoke = Run+Wait+Release into a single call
  finally
